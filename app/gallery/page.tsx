...
export default function GalleryPage() {
  ...
  useEffect(() => {
    ...
    if (ctx) {
      ...
      // Add event images as colored circles
      const events = [
        { name: 'AI Summit', x: 256, y: 256, color: '#D4A574', size: 80 },
        { name: 'Tech Expo', x: 512, y: 512, color: '#E97132', size: 70 },
        { name: 'Metaverse', x: 1024, y: 256, color: '#D4A574', size: 75 },
        { name: 'Blockchain', x: 1536, y: 512, color: '#D97706', size: 70 },
        { name: 'Security', x: 768, y: 768, color: '#E97132', size: 65 },
        { name: 'Digital Art', x: 1280, y: 800, color: '#D4A574', size: 70 }
      ]
      ...
      events.forEach(event => {
        ...
        ctx.globalAlpha = 0.3
        ctx.beginPath()
        ctx.arc(event.x, event.y, event.size + 15, 0, Math.PI * 2)
        ctx.stroke()
        ctx.globalAlpha = 1
        ...
        ctx.fillText(event.name, event.x, event.y)
      })
    }
  ...
  return (
    <main className="min-h-screen bg-black">
      ...
      <div className="pt-20 w-full h-screen flex flex-col items-center justify-center">
        <div ref={containerRef} className="w-full h-4/5" />
      ...
      <div className="fixed bottom-0 w-full z-40 border-t border-amber-700/20 backdrop-blur-md bg-black/50 py-6 px-6">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="text-xs text-gray-500 font-mono">
            <span className="text-amber-500">[ VARNOTHsava 2026 ]</span>
          </div>
        </div>
      </div>
    </main>
